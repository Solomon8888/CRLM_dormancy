# GSE282081差异表达火山图
#
# 批量读取主差异分析和ATF3组内高低表达差异的all_genes.csv，
# 按GSE114012的03号火山图风格输出PDF和PNG。
...
ANALYSES_TO_PLOT <- parse_env_vector("GSE282081_VOLCANO_ANALYSES", "all")
if (length(ANALYSES_TO_PLOT) == 1 && tolower(ANALYSES_TO_PLOT) == "all") {
  ANALYSES_TO_PLOT <- "all"
}
CATEGORIES_TO_PLOT <- parse_env_vector("GSE282081_VOLCANO_CATEGORIES", "all")
...
cat("\nRunning GSE282081 volcano plot generation...\n")
...
cat("\nGSE282081 volcano plot generation finished.\n")
