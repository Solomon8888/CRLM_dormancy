# GSE282081差异分析GSEA
#
# 自动读取02号脚本输出的每套DEG/all_genes.csv，使用limma t统计量作为排序
# 指标运行MSigDB GSEA。结果结构与GSE114012保持一致：
# results/ngs/GSE282081/tables/<analysis_name>/GSEA/<geneset>/gsea_result.csv
...
DEG_SCRIPT <- "scripts/GSE282081/02_limma_differential_expression.R"
...
OUTPUT_ROOT <- Sys.getenv("GSE282081_GSEA_OUTPUT_ROOT", unset = RESULT_ROOT)
TABLE_OUTPUT_ROOT <- file.path(OUTPUT_ROOT, "tables")
PLOT_ROOT <- file.path(OUTPUT_ROOT, "plots", "GSEA")

ANALYSES_TO_RUN <- parse_env_vector("GSE282081_DE_GSEA_ANALYSES", "all")
if (length(ANALYSES_TO_RUN) == 1 && tolower(ANALYSES_TO_RUN) == "all") {
  ANALYSES_TO_RUN <- "all"
}

SPECIES <- Sys.getenv("GSE282081_DE_GSEA_SPECIES", unset = "human")
GENE_ID_TYPE <- Sys.getenv("GSE282081_DE_GSEA_GENE_ID_TYPE", unset = "ENTREZ")
RANK_METRIC_COLUMN <- Sys.getenv("GSE282081_DE_GSEA_RANK_COLUMN", unset = "t")

GSEA_SIGNIFICANCE_COLUMN <- Sys.getenv("GSE282081_DE_GSEA_P_COLUMN", unset = "pvalue")
GSEA_SIGNIFICANCE_CUTOFF <- as.numeric(Sys.getenv("GSE282081_DE_GSEA_P_CUTOFF", unset = "0.05"))
GSEA_PARAMS <- list(
  exponent = 1,
  minGSSize = parse_env_integer("GSE282081_DE_GSEA_MIN_GS_SIZE", 5L),
  maxGSSize = parse_env_integer("GSE282081_DE_GSEA_MAX_GS_SIZE", 500L),
  pvalueCutoff = GSEA_SIGNIFICANCE_CUTOFF,
  pAdjustMethod = "BH",
  verbose = parse_env_logical("GSE282081_DE_GSEA_VERBOSE", FALSE),
  nPerm = 1000,
  method = "multilevel",
  adaptive = FALSE,
  minPerm = 101,
...
READABLE_GENE_SYMBOLS <- parse_env_logical("GSE282081_DE_GSEA_READABLE_SYMBOLS", TRUE)
USE_QS2_CACHE <- parse_env_logical("GSE282081_DE_GSEA_USE_QS2_CACHE", TRUE)
REFRESH_QS2_CACHE <- parse_env_logical("GSE282081_DE_GSEA_REFRESH_QS2_CACHE", TRUE)
CLEAN_GSEA_OUTPUT_DIR <- parse_env_logical("GSE282081_DE_GSEA_CLEAN", TRUE)
...
MSIGDB_REFERENCE_MAX_AGE_DAYS <- parse_env_integer("GSE282081_DE_GSEA_MSIGDB_MAX_AGE_DAYS", 7L)
...
cat("\nGSE282081 DE-GSEA runtime configuration:\n")
...
cat("\nRunning GSE282081 batch DE-GSEA analyses...\n")
...
stop_on_parallel_errors(summary_records, task_ids = task_ids, label = "GSE282081 DE-GSEA tasks")
...
cat("\nGSE282081 DE-GSEA compute summary:\n")
print(
  summary_table[
    ,
    c(
...
cat("\nGSE282081 batch DE-GSEA analysis finished.\n")
