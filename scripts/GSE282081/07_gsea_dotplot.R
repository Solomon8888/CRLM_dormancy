# GSE282081批量GSEA气泡图
#
# 扫描主差异GSEA、ATF3相关性GSEA和ATF3高低表达GSEA结果表，
# 将每个analysis x geneset结果绘制为气泡图并分门别类保存。
...
GSEA_DOTPLOT_P_COLUMN <- Sys.getenv("GSE282081_GSEA_DOTPLOT_P_COLUMN", unset = "pvalue")
GSEA_DOTPLOT_P_CUTOFF <- as.numeric(Sys.getenv("GSE282081_GSEA_DOTPLOT_P_CUTOFF", unset = "0.05"))
...
GSEA_CATEGORIES_TO_PLOT <- parse_env_vector("GSE282081_GSEA_DOTPLOT_CATEGORIES", "all")
GSEA_GENESETS_TO_PLOT <- parse_env_vector("GSE282081_GSEA_DOTPLOT_GENESETS", "all")
SHOW_NON_SIGNIFICANT_TOP_TERMS <- parse_env_logical(
  "GSE282081_GSEA_DOTPLOT_SHOW_NONSIG",
  SHOW_NON_SIGNIFICANT_TOP_TERMS
)
CLEAN_GSEA_DOTPLOT_ROOT <- parse_env_logical("GSE282081_GSEA_DOTPLOT_CLEAN", TRUE)
...
cat("\nRunning GSE282081 GSEA dotplot generation...\n")
...
cat("\nGSE282081 GSEA dotplot generation finished.\n")
