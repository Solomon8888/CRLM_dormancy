...
# 临时换基因可使用：
# TCGA_MEDIAN_DE_TARGET_GENES=ATF3,MYC Rscript scripts/quickanalysis/03_local_tcga_01a_median_de_gsea.R
TARGET_GENES <- parse_env_vector("TCGA_MEDIAN_DE_TARGET_GENES", c("ATF3"))

# 指定TCGA癌种。默认只分析COAD，不包含READ。
# 多癌种可写成：TCGA_MEDIAN_DE_TARGET_CANCERS=COAD,READ
TARGET_CANCERS <- parse_env_vector("TCGA_MEDIAN_DE_TARGET_CANCERS", c("COAD"))
...
SAMPLE_DETAIL_FILTER <- parse_env_vector(
  "TCGA_MEDIAN_DE_SAMPLE_DETAIL",
  c("tumor_primary_01a")
)
SAMPLE_BARCODE_PATTERN <- Sys.getenv(
  "TCGA_MEDIAN_DE_SAMPLE_PATTERN",
  unset = "-01A$"
)

# 目标基因分组使用的表达矩阵。TPM取log2(TPM + 1)后分组；单调变换不改变中位数分组方向。
TARGET_EXPRESSION_ASSAY <- Sys.getenv("TCGA_MEDIAN_DE_TARGET_ASSAY", unset = "tpm")
TARGET_EXPRESSION_LOG2 <- parse_env_logical("TCGA_MEDIAN_DE_TARGET_LOG2", TRUE)

# 差异分析使用raw counts，并仅保留protein-coding基因。
COUNT_ASSAY_NAME <- Sys.getenv("TCGA_MEDIAN_DE_COUNT_ASSAY", unset = "counts")
GENE_BIOTYPE_FILTER <- "coding"

# 中位数分组标签。阈值规则：表达值 >= median为High，表达值 < median为Low。
HIGH_GROUP_LABEL <- Sys.getenv("TCGA_MEDIAN_DE_HIGH_LABEL", unset = "High")
LOW_GROUP_LABEL <- Sys.getenv("TCGA_MEDIAN_DE_LOW_LABEL", unset = "Low")
MIN_SAMPLES_PER_GROUP <- parse_env_integer("TCGA_MEDIAN_DE_MIN_GROUP_N", 5L)

# limma显著差异筛选阈值。GSEA使用all_genes.csv中的t统计量，不受显著阈值限制。
P_VALUE_COLUMN <- Sys.getenv("TCGA_MEDIAN_DE_P_VALUE_COLUMN", unset = "P.Value")
P_VALUE_CUTOFF <- as.numeric(Sys.getenv("TCGA_MEDIAN_DE_P_VALUE_CUTOFF", unset = "0.05"))
LOGFC_CUTOFF <- as.numeric(Sys.getenv("TCGA_MEDIAN_DE_LOGFC_CUTOFF", unset = "0.5"))
OUTPUT_DROP_COLUMNS <- c("Feature_ID", "Biotype", "Length")


# 2. GSEA配置 -----------------------------------------------------------------

SPECIES <- Sys.getenv("TCGA_MEDIAN_DE_SPECIES", unset = "human")
GENE_ID_TYPE <- Sys.getenv("TCGA_MEDIAN_DE_GENE_ID_TYPE", unset = "ENTREZ")
RANK_METRIC_COLUMN <- Sys.getenv("TCGA_MEDIAN_DE_RANK_COLUMN", unset = "t")

GSEA_SIGNIFICANCE_COLUMN <- Sys.getenv("TCGA_MEDIAN_DE_GSEA_P_COLUMN", unset = "pvalue")
GSEA_SIGNIFICANCE_CUTOFF <- as.numeric(Sys.getenv("TCGA_MEDIAN_DE_GSEA_P_CUTOFF", unset = "0.05"))

GSEA_PARAMS <- list(
  exponent = 1,
  minGSSize = parse_env_integer("TCGA_MEDIAN_DE_GSEA_MIN_GS_SIZE", 5L),
...
# TCGA_MEDIAN_DE_GSEA_GENESETS=H Rscript scripts/quickanalysis/03_local_tcga_01a_median_de_gsea.R
GSEA_GENESETS_TO_RUN <- parse_env_vector(
  "TCGA_MEDIAN_DE_GSEA_GENESETS",
  c(
    "H",
    "C2:CP:BIOCARTA",
    "C2:CP:KEGG_MEDICUS",
...
READABLE_GENE_SYMBOLS <- parse_env_logical("TCGA_MEDIAN_DE_READABLE_SYMBOLS", TRUE)
USE_QS2_CACHE <- parse_env_logical("TCGA_MEDIAN_DE_USE_QS2_CACHE", TRUE)
REFRESH_QS2_CACHE <- parse_env_logical("TCGA_MEDIAN_DE_REFRESH_QS2_CACHE", TRUE)
MSIGDB_REFERENCE_MAX_AGE_DAYS <- parse_env_integer("TCGA_MEDIAN_DE_MSIGDB_MAX_AGE_DAYS", 7L)

# GSEA绘图配置：默认输出每类基因集的dotplot。
# 单通路GSEA/表达热图非常耗时，quickanalysis默认关闭；
# 如需完全复刻NGS 07号脚本的TopN单通路图，可设置TCGA_MEDIAN_DE_DRAW_SINGLE_PATHWAY=1。
RUN_GSEA_PLOTS <- parse_env_logical("TCGA_MEDIAN_DE_RUN_GSEA_PLOTS", TRUE)
DRAW_SINGLE_PATHWAY_GSEA <- parse_env_logical("TCGA_MEDIAN_DE_DRAW_SINGLE_PATHWAY", FALSE)
SINGLE_PATHWAY_TOP_N <- parse_env_integer("TCGA_MEDIAN_DE_SINGLE_PATHWAY_TOP_N", 10L)
SINGLE_PATHWAY_KEYWORDS <- parse_env_vector("TCGA_MEDIAN_DE_SINGLE_PATHWAY_KEYWORDS", character(0))
SINGLE_PATHWAY_MAX_KEYWORD_TERMS <- parse_env_integer("TCGA_MEDIAN_DE_SINGLE_PATHWAY_MAX_KEYWORD", 20L)
...
ANALYSIS_MODULE <- "03_median_de_gsea"
...
run_one_median_de_analysis <- function(task_id) {
  ...
  analysis_name <- sanitize_file_name(
    paste(cancer, "01A", target_gene, "median_high_vs_low", sep = "_")
  )
  ...
  sample_group_table <- data.frame(
    ...
    Patient_ID = if ("Patient_ID" %in% colnames(sample_info_used)) sample_info_used$Patient_ID else NA_character_,
