...
# 6. 结果统一保存到results/local_tcga/<目标基因>-gtex/06_correlation_gsea。
#
# 默认示例：
# - 目标基因：ATF3
# - 组织：COLON
...
# 临时换基因可使用：
# GTEX_CORRELATION_TARGET_GENES=ATF3,MYC Rscript scripts/quickanalysis/06_local_gtex_correlation_gsea.R
TARGET_GENES <- parse_env_vector("GTEX_CORRELATION_TARGET_GENES", c("ATF3"))

# 指定GTEx组织数据集。默认分析COLON。
TARGET_TISSUES <- parse_env_vector("GTEX_CORRELATION_TARGET_TISSUES", c("COLON"))

# 本地SE文件模板。默认读取data/GTEx/<组织>/data_prepare/<组织>_se_raw.rds。
SE_FILE_TEMPLATE <- NULL

# GTEx默认使用全部样本；如需筛选可配置样本注释列、取值或Sample_ID正则。
SAMPLE_FILTER_COLUMN <- Sys.getenv("GTEX_CORRELATION_SAMPLE_FILTER_COLUMN", unset = "")
SAMPLE_FILTER_VALUES <- parse_env_vector("GTEX_CORRELATION_SAMPLE_FILTER_VALUES", character(0))
SAMPLE_BARCODE_PATTERN <- Sys.getenv("GTEX_CORRELATION_SAMPLE_PATTERN", unset = "")

# 相关性分析使用的表达矩阵。默认使用log2(TPM + 1)。
EXPRESSION_ASSAY <- Sys.getenv("GTEX_CORRELATION_ASSAY", unset = "tpm")
EXPRESSION_LOG2 <- parse_env_logical("GTEX_CORRELATION_LOG2", TRUE)

# 相关性方法可选pearson或spearman。
CORRELATION_METHOD <- tolower(Sys.getenv("GTEX_CORRELATION_METHOD", unset = "spearman"))
stopifnot(CORRELATION_METHOD %in% c("pearson", "spearman"))

# 仅保留protein-coding基因；目标基因本身默认不纳入“其他基因”的相关性排序。
COUNT_ASSAY_NAME <- Sys.getenv("GTEX_CORRELATION_COUNT_ASSAY", unset = "counts")
GENE_BIOTYPE_FILTER <- "coding"
EXCLUDE_TARGET_GENE_FROM_RANKING <- parse_env_logical("GTEX_CORRELATION_EXCLUDE_TARGET", TRUE)
MIN_COMPLETE_SAMPLES <- parse_env_integer("GTEX_CORRELATION_MIN_COMPLETE_N", 10L)

# 相关性显著性筛选阈值。GSEA使用all_genes.csv中的Correlation排序，不受显著阈值限制。
P_VALUE_COLUMN <- Sys.getenv("GTEX_CORRELATION_P_VALUE_COLUMN", unset = "P.Value")
P_VALUE_CUTOFF <- as.numeric(Sys.getenv("GTEX_CORRELATION_P_VALUE_CUTOFF", unset = "0.05"))
CORRELATION_CUTOFF <- as.numeric(Sys.getenv("GTEX_CORRELATION_EFFECT_CUTOFF", unset = "0.3"))
TOP_CORRELATED_N <- parse_env_integer("GTEX_CORRELATION_TOP_N", 100L)
OUTPUT_DROP_COLUMNS <- c("Feature_ID", "Biotype", "Length")

# 单通路GSEA表达热图仍需要样本分组展示顺序；这里仅用于热图排序，不参与相关性计算。
HIGH_GROUP_LABEL <- Sys.getenv("GTEX_CORRELATION_HIGH_LABEL", unset = "High")
LOW_GROUP_LABEL <- Sys.getenv("GTEX_CORRELATION_LOW_LABEL", unset = "Low")


# 2. GSEA配置 -----------------------------------------------------------------

SPECIES <- Sys.getenv("GTEX_CORRELATION_SPECIES", unset = "human")
GENE_ID_TYPE <- Sys.getenv("GTEX_CORRELATION_GENE_ID_TYPE", unset = "ENTREZ")
RANK_METRIC_COLUMN <- Sys.getenv("GTEX_CORRELATION_RANK_COLUMN", unset = "Correlation")

GSEA_SIGNIFICANCE_COLUMN <- Sys.getenv("GTEX_CORRELATION_GSEA_P_COLUMN", unset = "pvalue")
GSEA_SIGNIFICANCE_CUTOFF <- as.numeric(Sys.getenv("GTEX_CORRELATION_GSEA_P_CUTOFF", unset = "0.05"))

GSEA_PARAMS <- list(
  exponent = 1,
  minGSSize = parse_env_integer("GTEX_CORRELATION_GSEA_MIN_GS_SIZE", 5L),
  maxGSSize = parse_env_integer("GTEX_CORRELATION_GSEA_MAX_GS_SIZE", 500L),
...
# GTEX_CORRELATION_GSEA_GENESETS=H Rscript scripts/quickanalysis/06_local_gtex_correlation_gsea.R
GSEA_GENESETS_TO_RUN <- parse_env_vector(
  "GTEX_CORRELATION_GSEA_GENESETS",
  c(
    "H",
    "C2:CP:BIOCARTA",
    "C2:CP:KEGG_MEDICUS",
...
READABLE_GENE_SYMBOLS <- parse_env_logical("GTEX_CORRELATION_READABLE_SYMBOLS", TRUE)
USE_QS2_CACHE <- parse_env_logical("GTEX_CORRELATION_USE_QS2_CACHE", TRUE)
REFRESH_QS2_CACHE <- parse_env_logical("GTEX_CORRELATION_REFRESH_QS2_CACHE", TRUE)
MSIGDB_REFERENCE_MAX_AGE_DAYS <- parse_env_integer("GTEX_CORRELATION_MSIGDB_MAX_AGE_DAYS", 7L)

# GSEA绘图配置：默认输出每类基因集的dotplot。
# 单通路GSEA/表达热图非常耗时，quickanalysis默认关闭；
# 如需完全复刻NGS 07号脚本的TopN单通路图，可设置GTEX_CORRELATION_DRAW_SINGLE_PATHWAY=1。
RUN_GSEA_PLOTS <- parse_env_logical("GTEX_CORRELATION_RUN_GSEA_PLOTS", TRUE)
DRAW_SINGLE_PATHWAY_GSEA <- parse_env_logical("GTEX_CORRELATION_DRAW_SINGLE_PATHWAY", FALSE)
SINGLE_PATHWAY_TOP_N <- parse_env_integer("GTEX_CORRELATION_SINGLE_PATHWAY_TOP_N", 10L)
SINGLE_PATHWAY_KEYWORDS <- parse_env_vector("GTEX_CORRELATION_SINGLE_PATHWAY_KEYWORDS", character(0))
SINGLE_PATHWAY_MAX_KEYWORD_TERMS <- parse_env_integer("GTEX_CORRELATION_SINGLE_PATHWAY_MAX_KEYWORD", 20L)
...
ANALYSIS_MODULE <- "06_correlation_gsea"
...
cat("\nLocal GTEx target-gene correlation + GSEA configuration:\n")
...
run_one_correlation_analysis <- function(task_id) {
  ...
  analysis_name <- sanitize_file_name(
    paste(tissue, "GTEx", target_gene, CORRELATION_METHOD, "correlation", sep = "_")
  )
  ...
  sample_expression_table <- data.frame(
    ...
    Subject_ID = if ("Subject_ID" %in% colnames(sample_info)) sample_info$Subject_ID else NA_character_,
...
print(
  analysis_summaries[
    ,
    c(
      "Tissue", "Target_Gene", "Samples_Used", "Ranked_Genes",
...
cat("\nLocal GTEx target-gene correlation + GSEA analysis finished.\n")
